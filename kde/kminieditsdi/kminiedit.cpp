...
void KMiniEdit::fileOpen()
{
  ...
  if (!newurl.isEmpty())
  {
    if (url.isEmpty() && !edit->edited())
      loadFile(newurl);
    else
    {
      KMiniEdit *window = new KMiniEdit();
      window->show();
      window->loadFile(newurl);
    }
  }
}
...
void KMiniEdit::addURLtoRecent()
{
 ...
 if (memberList)
  {
    for (uint i=0; i<memberList->count(); i++)
    {
      KMiniEdit *w = (KMiniEdit *) (memberList->at(i));
      w->recentAction->loadEntries(KGlobal::config());
    }
  }
}
