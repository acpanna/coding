...
\noindent \textcolor{black}{\underbar{\Huge Generalvoraussetzungen:}}\\
\\
 \textcolor{black}{\huge Seien $K$ ein Körper, $R$ ein euklidischer
Ring, $V$ ein $K$-Vektorraum mit $dimV=n\:(n\in\mathbb{N})$, $\tau\in End(V)$
, und $M$ ein eindlich erzeugter Modul über $R$.}{\huge \par}
...
$O(m)$ ist ein Ideal. 
\item Ein Element $m\in M$ heißt Torsionselement, wenn $O(m)\supsetneqq\{0\}$.
Ist jedes $m\in M$ Torsionselement, so heißt $M$ Torsionsmodul.
\vspace*{\fill}
\end{enumerate}
...
1. Für Elemente $k\in K\subset K[X]$ folgt aus diese Definition:
...
Sei $v\in V$. Das Polynom $o\in K[X]$ heißt Ordnungspolynom von
$v$, wenn gilt: \\
...
und $Env(V)=Mat(3\times3,\mathbb{Z}_{5})$.\\
...
3\\
0\end{array}\right)$.\\
Bestimmes $s$ minimal, so daß $v,Av,\ldots,A^{s}v$ linear abhängig
sind. Hier $s=3$.\\
...
\begin{enumerate}
\item Die Untermoduln von $V_{\tau}$ sind gerade diejenigen Unterräume
$U$ von $V$ die $\tau(U)\subset U$ ( d.h. $U$ ist $\tau$-invariant
) erfüllen , hier bezeichnet mit $U_{\tau}$. 
\item Der Endomorphismus $\tau$ muss nun verändert werden: Setze \[
...
zum Schluss der Rechnung den Übergang modulo $U_{\tau}$. \vspace*{\fill}
\end{enumerate}
...
linear unabhängig sind ( dies wird mit der Funktion gauss getestet
) : setzte $i:=i+1$ . \\
 Setze $m:=i$.\\
...
\textcolor{blue}{\Huge 2. Der Algorithmus Minpoly}{\Huge \par}
...
Sei $\tau\in End(V)$. Das Polynom $m\in K[X]$ heißt Minimalpolynom
...
$d:=o_{1}=(X+2)(X+3)=X^{2}+1$

\textcolor{black}{Berechne: \\*[-60pt]}
...
T & := & \ggT(C,D)=X+2\\
D_{2} & := & \frac{D}{T}=X+3\\
m & := & CD_{2}=(X+2)(X+3)(X+4)=X^{3}+4X^{2}+X+4\end{eqnarray*}
...
m & := & CD_{2}=(X+2)(X+3)(X+4)^{2}=X^{4}+3X^{3}+2X^{2}+3X+1\end{eqnarray*}
\vspace*{\fill}
...
der Theorie nicht mehr größer werden.
...
nur über das Erzeugendensystem $E$ zu bilden:\[
...
Sei $o_{i}\in K[X]$ das Ordnungspolynom von $e_{i}$ für $i=0,\ldots,k$
...
\begin{Sat}
Seien \(v_{0}, v_{1} \in M\) Torsionselemente. Ferner sei \(O(v_{0}) = R c \) und \(O(v_{1}) = R d\). Setze:   \( \quad  C:=r(c, \frac{d}{\ggT(c,d)} ) \: , \: D:= r(d, \frac{c}{\ggT(c,d)} )\) und \( D_{2}:=\frac{D}{\ggT(C,D)}\). Setze ferner \(v:= \frac{c}{C} v_{0} + \frac{d}{D_{2}} v_{1}\). Dann gilt: \[ O(v) = O(v_{0}) \cap O(v_{1}) = R \cdot \kgV(c,d) = Rc \cap Rd \]
\end{Sat}
...
Sei $v_{0},v_{1},\ldots,v_{n-1}$ eine Basis von $V$. Dann ist $\overline{v}_{0},\overline{v}_{1},\ldots,\overline{v}_{n-1}$
ein Erzeugendensystem von $V_{\tau}/U_{\tau}$ ( auch $U_{\tau}=\{0\}$
möglich ). \\


\textbf{Vorarbeit:}~ Berechne Ordnungspolynom von $v_{i}$ und speichere
es in $ordpol[i]$ für $\: i=0,\ldots,n-1$ . 

\textbf{Schritt~0:}~ Setze $v$ und $m$ wie folgt: \begin{eqnarray*}
v & := & v_{0}\\
...
\textbf{Schritt~i:}~ ( Für $i=1,\ldots,n-1$ ) \\
...
v:= & \frac{c}{C}v+\frac{d}{D_{2}}v_{i} & \quad(\textrm{ d}.\textrm{h}.\: O(v)=K[X]m\:\:)\end{eqnarray*}
...
Es ist $m=\kgV(\, ordpol[0],\ldots,ordpol[n-1]\,)$ Also ist $K[X]m=\bigcap_{i=0}^{n-1}K[X]ordpol[i]=\bigcap_{i=0}^{n}O(v_{i})=A(V_{\tau})\:$
nach den Sätzen 1 und 2 . Damit ist $m$ das Minimalpolynom von $\tau$
nach der Definition 1. 

Es ist $O(v)=\bigcap_{i=0}^{n-1}{O(v_{i})}=K[X]m$ nach Satz 4. Also
hat $v$ das Polynom $m$ als Ordnungspolynom. 
